    ...
    PerformHttpRequest(('%s/main/lua/fxmanifest.lua'):format(url:gsub('github.com', 'raw.githubusercontent.com')), function(error, response)
        if error == 200 then
            ...
            if version ~= latest then
                ...
                local link = ('%s/releases/download/v%s/ghmattimysql-v%s.zip'):format(url, latest, latest)
                ...
                print(('^3%s is available for ghmattimysql-oxmysql - please update to the latest release (current version: %s)\n   ^3- %s^0'):format(latest, version, link))
            end
        end
    end, 'GET')
end)
...
exports('executeSync', function(query, parameters)
    local p = promise.new()
    ghmattimysql:execute(query, parameters, function(result)
        p:resolve(result)
...
exports('fetchSync', function(query, parameters)
    local p = promise.new()
    ghmattimysql:fetch(query, parameters, function(result)
        p:resolve(result)
    end, GetInvokingResource())
    return Citizen.Await(p)
end)
...
exports('singleSync', function(query, parameters)
    local p = promise.new()
    ghmattimysql:single(query, parameters, function(result)
        p:resolve(result)
    end, GetInvokingResource())
    return Citizen.Await(p)
end)
...
exports('scalarSync', function(query, parameters)
    local p = promise.new()
    ghmattimysql:scalar(query, parameters, function(result)
        p:resolve(result)
    end, GetInvokingResource())
    return Citizen.Await(p)
end)
...
exports('insertSync', function(query, parameters)
    local p = promise.new()
    ghmattimysql:insert(query, parameters, function(result)
        p:resolve(result)
    end, GetInvokingResource())
    return Citizen.Await(p)
end)
...
exports('updateSync', function(query, parameters)
    local p = promise.new()
    ghmattimysql:update(query, parameters, function(result)
        p:resolve(result)
    end, GetInvokingResource())
    return Citizen.Await(p)
end)
...
exports('transactionSync', function(queries, parameters)
    local p = promise.new()
    ghmattimysql:transaction(queries, parameters, function(result)
        p:resolve(result)
    end, GetInvokingResource())
    return Citizen.Await(p)
end)
...
exports('prepareSync', function(query, parameters)
    local p = promise.new()
    ghmattimysql:prepare(query, parameters, function(result)
        p:resolve(result)
    end, GetInvokingResource())
    return Citizen.Await(p)
end)